    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-slate-900">Pagar via PIX</h2>
...
function CheckoutPageInner() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      amountBrl: finalPriceInCents / 100,
    }
    ...
    try {
      if (paymentMethod === 'pix') {
        ...
        // Salvar dados na sessionStorage para a página de pagamento
        sessionStorage.setItem('stripe_payment_intent', JSON.stringify({
                  ...
                  <div className="flex gap-4">
                    ...
                    >
                      {loading
                        ? 'Processando...'
                        : paymentMethod === 'pix'
                          ? `Gerar PIX ${formatCurrencyFromCents(finalPriceInCents)}`
                          : `Pagar com cartão ${formatCurrencyFromCents(finalPriceInCents)}`}
                    </button>
                  </div>
                </form>
              )}
